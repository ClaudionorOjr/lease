        ...
        <Card key={solicitation.id} className="max-w-xl ">
          ...
          <CardContent className="grid grid-cols-4  gap-4">
            ...
            <div className="col-span-2 ">
              <Label>Email</Label>
              {solicitation.email ? (
                <p className="text-muted-foreground">{solicitation.email}</p>
              ) : (
          ...
          <CardFooter className="gap-4">
            {solicitation.status === 'PENDING' ? (
              <>
                <form
                  action={acceptSolicitationAction.bind(null, solicitation.id)}
                >
                  <Button type="submit">Accept</Button>
                </form>
                <form
                  action={refuseSolicitationAction.bind(null, solicitation.id)}
                >
